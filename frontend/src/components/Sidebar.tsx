...
export function Sidebar({ role }: { role: Role }) {
  ...
  const item = (nav: { href: string; label: string; icon: string }) => {
    ...
    return (
      ...
      >
        {on && (
          <span className="absolute left-0 top-2 bottom-2 w-[2px] rounded-full bg-[#fc72ff]" />
        )}
        <Icon d={nav.icon} />
      ...
      >
        ...
        >
          <Link
            ...
            className="relative h-7 w-7 shrink-0 ml-4"
          >
            <Image
              src="/logo.png"
              alt="Logo"
              ...
              {!collapsed && (
                <div className="mt-4 rounded-xl border border-white/[0.06] bg-white/[0.03] p-3">
                  ...
                  <p className="mt-1 text-[11px] leading-relaxed text-white/30">
                    CEO wallet unlocks admin tools. Employees use an access
                    code.
                  </p>
                </div>
              )}
        ...
        <div className="border-t border-white/[0.06] p-2.5">
          ...
          >
            <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-[#fc72ff]/10">
              <svg
                className="h-3.5 w-3.5 text-[#fc72ff]"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path
